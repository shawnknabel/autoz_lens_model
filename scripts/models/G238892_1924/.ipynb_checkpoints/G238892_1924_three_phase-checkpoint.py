...
# # 4/7/21 - 3-phase fit to G238892_1924.

# ## Phases: 
#1(r) phase1_nlive = 200, phase1_tolerance = 0.5
#2(r) phase2_nlive = 200 (source mask), phase2_tolerance = 0.25
#3(r) phase3_nlive = 300, phase3_tolerance = 0.25, facc=0.3 for all phases, use effective radius from GAMA DR3 Sersic Photometry catalog (including errors) and upper limit (effective radius + 3*error), 4 position, threshold=1.0
# 
...
# set up masks
mask = al.Mask2D.from_fits(f'{object_folder}{links_id}_r_mask.fits', pixel_scales=imaging_r.pixel_scales)
lens_mask = al.Mask2D.from_fits(f'{object_folder}{links_id}_r_lens_mask.fits', pixel_scales=imaging_r.pixel_scales)
source_mask = al.Mask2D.from_fits(f'{object_folder}{links_id}_r_source_mask.fits', pixel_scales=imaging_r.pixel_scales)
...
# set positions
# phase 2 is g
pos_r = np.genfromtxt(f'{object_folder}{links_id}_r_positions_grid.csv', delimiter=',', skip_header=0)
imaging_g.positions = al.Grid2DIrregular(
    [(pos_r[0]), (pos_r[1]), (pos_g[2])]#, (pos_g[3]), (pos_g[4]),]
)
