...
# # 4/7/21 - 3-phase fit to G165293_2144.

# ## Phases: 
#1(r) phase1_nlive = 200, phase1_tolerance = 0.5
#2(g) phase2_nlive = 200 (source mask), phase2_tolerance = 0.25
#3(g) phase3_nlive = 300, phase3_tolerance = 0.25, facc=0.3 for all phases, use effective radius from GAMA DR3 Sersic Photometry catalog (including errors) and upper limit (effective radius + 3*error), 4 position, threshold=1.0
# 
...
#Set up lens mask for phase 1

lens_mask = al.Mask2D.elliptical(
    shape_native=imaging_r.shape_native, pixel_scales=imaging_r.pixel_scales, sub_size=2,
    major_axis_radius=2.6,
    axis_ratio=0.6,
    phi=100.0,
    centre=(0.0,0.1)
)
...
source_mask = al.Mask2D.elliptical_annular(
    ...
    centre=(0.0, 0.1),
    inner_major_axis_radius=2.6,
    inner_axis_ratio=0.6,
    inner_phi=100.0,
    outer_major_axis_radius=3.7,
...
imaging_g.positions = al.Grid2DIrregular(
    [(pos_g[0]), (pos_g[1]), (pos_g[2])]#, (pos[3])]#, (pos[4]),]
)
...
#    [(pos_white[0]), (pos_white[1]), (pos_white[2])]#, (pos[3])]#, (pos[4]),]
#)
...
lens.mass.einstein_radius = af.GaussianPrior(mean=einstein_radius, sigma=0.5*einstein_radius, 
                                             lower_limit=0.0) # take sigma to be 50% of mean # hmmm
